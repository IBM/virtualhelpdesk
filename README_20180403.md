...
# **`Virtual HelpDesk Components`**

The `Virtual HelpDesk`has three main components.
...
The Virtual HelpDesk can perform a list of tasks.


### **`Q/A session delivered by Watson Assistance(Conversation) service`**

End users may have question and answer interactive conversation with the virtual agent. As long as the virtual agent is trained to cover the subjects, the Q/A session is easy and straight forward. You may type problem statements such as
...
### **`Knowledge Base search through Watson Discovery service`**
...
If the entries from the knowledge base still does not provide sufficient information to end users, users have option to open ticket.
